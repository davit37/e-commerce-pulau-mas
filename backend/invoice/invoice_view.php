...
if (isset($_GET['act'])) {
	...
	$sql = "select idproduk,jumla from transaksi where  noinvoice='$id' ";
	$res=mysql_query($sql) or die(mysql_error());
	while($rows=mysql_fetch_object($res)){
		//2. kurangi barang sejumlah pembelian 
		$query="update stok set jumlah=jumlah - ".$rows->jumla." where idproduk=".$rows->idproduk;
...
<div>
    ...
    <form action="" method="post" name="form1" class="form-horizontal">
                  ...
                  <a href="cetak_transaksi.php?tgl_awal=<?php echo $_POST['tgl_awal']; ?>&tgl_akhir=<?php echo $_POST['tgl_akhir']; ?>" target="_blank" class="btn btn-success btn-sm"> Cetak</a> 
                  </form>
...
$a = $_POST['tgl_awal'];
 ...
 limit $posisi,$batas ";
	...
	else
	{
		$query = "select invoice.*,pelanggan.nama,alamat_kirim.total_transaksi
 from invoice,pelanggan,alamat_kirim
 where invoice.idpelanggan=pelanggan.idpelanggan and invoice.noinvoice=alamat_kirim.noinvoice and  invoice.tanggal >= '$a' and invoice.tanggal <='$b'  ";
	}
