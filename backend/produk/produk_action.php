...
if($aksi == 'tambah') {
	$sql = "INSERT INTO produk(nama_produk,berat_produk,idkategori,foto,
	deskripsi)
		VALUES('$nama_produk','$berat_produk',
		'$idkategori','$foto_file','$deskripsi')";
}else if($aksi== 'edit') {
	if($ukuran_file > 0){
	$sql = "update produk set nama_produk='$nama_produk',berat_produk='$berat_produk',foto='$foto_file',
	idkategori='$idkategori',deskripsi='$deskripsi'
	where idproduk='$id'";

	}else{
		$sql = "update produk set nama_produk='$nama_produk',berat_produk='$berat_produk',
